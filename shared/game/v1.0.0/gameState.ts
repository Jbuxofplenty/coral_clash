...
export function restoreGameFromSnapshot(coralClash: any, snapshot: any): void {
    ...
    // Restore coral remaining counts FIRST (before placing coral)
    // This ensures we have the correct counts when placing coral
    ...
    // Note: We directly set _coral to avoid decrementing coralRemaining (which was already restored above)
    if (snapshot.coral) {
        const Ox88 = {
            a8: 0x00,
            b8: 0x01,
        ...
        // IMPORTANT: Directly set _coral array instead of calling placeCoral()
        // to avoid decrementing _coralRemaining (which was already restored above)
