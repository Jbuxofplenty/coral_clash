...
export async function makeComputerMoveHelper(gameId, gameData = null) {
    ...
    if (difficulty !== 'random') {
        // We can't access maxTimeMs easily here without recalculating or restructuring, 
        // but we can log the actual time taken.
        // Recalculate what the limit likely was for logging purposes
        const idealLimit = calculateOptimalMoveTime(difficulty, timeRemainingMs);
        console.log(`[makeComputerMove] AI (${difficulty}) move stats: Taken=${moveCalculationTimeMs}ms, Limit=${idealLimit}ms, Moves=${moves.length}`);
    }
