...
class Product extends React.Component {
  render() {
    ...
    return (
      <TouchableWithoutFeedback onPress={() => navigation.navigate('Game', { product: product })}>
        <Block row={horizontal} card style={[styles.product, styles.shadow, style]}>
          ...
          <Block flex space="between" style={styles.productDescription}>
            <Text size={18} bold style={styles.productTitle}>{product.title}</Text>
            {product.description && (
              <Text size={14} color={theme.COLORS.MUTED} style={styles.productSubtitle}>
                {product.description}
              </Text>
            )}
          </Block>
